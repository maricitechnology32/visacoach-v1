...
// 1. (FIX) Import ALL necessary functions from the service
  ...
  return (
    <div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {consultancies.map((c) => (
              <tr key={c._id}>
                ...
                {/* 2. (FIX) Consolidated all actions into one table cell */}
      ...
      <Modal isOpen={isCounselorModalOpen} onClose={() => setIsCounselorModalOpen(false)} title={`Add Counselor to ${selectedConsultancy?.name}`}>
        <form onSubmit={handleCreateCounselor} className="space-y-4">{/* ...form content... */}</form>
      </Modal>
    </div>
  );
};
